...
.sort((a, b) => {
  const conceptComparison = a.concept[0].localeCompare(b.concept[0]);
  if (conceptComparison === 0) {
    return a.strokes - b.strokes;
  }
  return conceptComparison;
}).sort((a, b) => {
  const numA = kanjiToNumber[a.kanji];
  const numB = kanjiToNumber[b.kanji];
  return numA - numB; // Sort by numeric value
});

export default N5_KANJI
